...
export class Game extends Level {
  ...
  wsStartGame(event, playerId) {
    ...
    this.components.forEach((component) => {
      if (component.state == 'active') {
        this.exec(component, 'onEnable', playerId);
      }
    });
  ...
  wsComponentClick(event, playerId) {
    ...
    console.log('onClick', component, playerId);
    this.exec(component, 'onClick', playerId);
    return ['components'];
  }
  wsComponentMouseover(event, playerId) {
    const component = this.getTarget(event.component);
    this.exec(component, 'onMouseover', playerId);
    return ['components'];
  }
  wsComponentMouseleave(event, playerId) {
    const component = this.getTarget(event.component);
    this.exec(component, 'onMouseleave', playerId);
    return ['components'];
  }
  wsComponentInput(event, playerId) {
    ...
    if (value != component.data.value) {
      ...
      this.exec(component, 'onInput', playerId);
    }
    return ['components'];
  }
}
